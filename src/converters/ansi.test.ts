...
// Identity class map for testing — maps logical names to themselves
const classMap: Record<string, string> = new Proxy({} as Record<string, string>, {
  get: (_target, prop: string) => prop,
});
const linkClass = 'test-link';
...
describe('ANSI to HTML Converter', () => {
  ...
  describe('256 Colors (38;5;n)', () => {
    test('256-color foreground uses CSS variable inline style', () => {
      const input = '\x1b[38;5;196mRed 256\x1b[0m';
      const { html } = convertAnsiToHtml(input, classMap, linkClass);
      expect(html).toContain('var(--ansi-color-196)');
      expect(html).toContain('Red 256');
    });
  });

  describe('Basic ANSI colors', () => {
    test('Basic red (31) uses CSS variable inline style', () => {
      const input = '\x1b[31mRed\x1b[0m';
      const { html } = convertAnsiToHtml(input, classMap, linkClass);
      expect(html).toContain('var(--ansi-color-1)');
